...
module ad_serdes_in #(
  ...
  if (FPGA_TECHNOLOGY == ULTRASCALE || FPGA_TECHNOLOGY == ULTRASCALE_PLUS) begin
    for (l_inst = 0; l_inst <= (DATA_WIDTH-1); l_inst = l_inst + 1) begin: g_data
      ...
      ISERDESE3 #(
        // RETEST 2026-08-04 with correct instrumentation. Hardcoded 8 while
        // ad408x_phy drives CLKDIV = clk/2, which delivers only 4 new bits per
        // CLKDIV. A deserializer expecting 8 bits per CLKDIV but clocked twice
        // as often repeats each bit in two consecutive output words - exactly
        // the 2x oversampling measured on the wire (40-bit period; decimate-by-2
        // gives a perfect 20-bit period). ISERDESE2 honours SERDES_FACTOR, which
        // is why the ZedBoard (identical 400 MHz clock plan) aligns and this does
        // not. Q mapping is left ALONE: in 4-bit mode the valid bits are Q[7:4],
        // where data_s0..s3 already sit.

# DAQ USB auto-update (Genesys ZU — Wildcat2 / EVAL-AD4080-FMC)
...
> **Forked from `projects/ad738x_fmc/genesys_zu5ev/usb_update`** (Wildcat /
> EVAL-AD738x) so the two boards can diverge. The engine, overlay and host
> tooling are otherwise identical — fixes worth having on both must be applied
> to both trees.
>
> Deliberately **not** carried over: `overlay/usr/local/bin/fmc-vadj-fix`. That
> script patches the **EVAL-AD7383-4FMCZ** FMC EEPROM (its FRU asked for 4 A on
> VADJ against a ~2.1 A budget) using values and checksums specific to that
> card, so running it against an AD4080 card would be wrong. If the
> EVAL-AD4080-FMC turns out to have the same over-budget FRU, add an
> AD4080-specific equivalent here rather than reusing the AD7383 one.
...
```bash
usb_update/host/make_update_bundle.sh 1.0.2 /path/to/BOOT.BIN [Image] [system.dtb]
```
...
usb_update/host/make_update_bundle.sh 1.0.2 Image BOOT.BIN system.dtb rootfs.tar \
    usb_update/post.sh
```

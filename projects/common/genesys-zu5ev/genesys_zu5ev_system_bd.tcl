...
###############################################################################
## Digilent Genesys ZU-5EV carrier base design (XCZU5EV-SFVC784-1-E).
## Modeled on projects/common/zcu102/zcu102_system_bd.tcl.
## Requires the Digilent board files (digilentinc.com:gzu_5ev) to be visible
## to Vivado (board.repoPaths) so apply_board_preset can configure the PS
## (DDR4, MIO, clocks) for this board.
...
# Genesys ZU PS_REF_CLK is 30 MHz (Digilent preset value) -- NOT the 33.333
# used by Xilinx eval boards. Wrong value here skews every PS-derived clock
# (UART baud, SD, DDR) by 10% and the board will not boot.
ad_ip_parameter sys_ps8 CONFIG.PSU__PSS_REF_CLK__FREQMHZ 30
ad_ip_parameter sys_ps8 CONFIG.PSU__USE__M_AXI_GP0 0
ad_ip_parameter sys_ps8 CONFIG.PSU__USE__M_AXI_GP1 0

# The Digilent board preset enables S_AXI ports (e.g. HPC0) that this design
# never clocks; disable them all -- ad_mem_hp*_interconnect re-enables the
...
# free-running 100 MHz clock for the VADJ_AUTO edge generator in system_top
create_bd_port -dir O -type clk vadj_clk
ad_connect  vadj_clk sys_ps8/pl_clk0
